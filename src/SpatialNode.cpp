...
SpatialNode::SpatialNode(int node_type, int sheet_id, int node_id, vector < double > coordenates, char * node_function)
{
...
string SpatialNode::getConnectionString()
{
	...
	{
		for(int i = 0; i < (int)inputs_nodes.size(); i++)
			connections << "\t\tConnection " << inputs_nodes.at(i)->GetNodeId() << ":\tWeight\t" << inputs_weight.at(i) << endl;
	}
	else if (node_type == 0)
