...
namespace ANN_USM{
	...
	public:
		 ...
		 */
		SpatialNode(int node_type, int sheet_id, int node_id, vector < double > coordenates, char * node_function);
		...
		/**
		 * \brief Get output function from this node recursively backwards. This functions is formated for its use in Octave or Matlab environment.
		 * \return Function in string format
		 */
		string GetOutputNodeFunction();
